...
for k=1:num_labels
	ny= y==k;
	nH=H2(:,k);
	J=J + (-ny' * log(nH) - (ones(m,1) - ny)' * log(ones(m,1) - nH));
end ;
J=J/m;	
temp1=Theta1 .^2;
temp2=Theta2.^2;
J=J+lambda*(sum(sum(temp1(:,2:end)))+sum(sum(temp2(:,2:end))))/(2*m);
...
for t = 1 : m
    ...
    d3 = a3;
    d3(y(t)) = d3(y(t)) - 1;% aas y(t) gives index

    d2 = d3*Theta2; 
    d2 = d2(2 : end); 
    d2 = d2 .* sigmoidGradient(a1* Theta1');
